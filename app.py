...
# ──────────────────────────────────────────────────────────────────────────────
# HELPERS  (NO CHANGES TO FORMULAS / WEIGHTS / GRADE SCALE)
# ──────────────────────────────────────────────────────────────────────────────
WEIGHTS = {
    "Assignments": 0.10,
    "Midterm Exam": 0.30,
...
def _load_demo_case_1():
    ...
    st.session_state.attendance_score = 90
...
with st.sidebar:
    ...
    st.markdown("<div class='sb-label'>📸 PPT-003 Demo Cases</div>", unsafe_allow_html=True)
    d1, d2 = st.columns(2)
    with d1:
        if st.button("Case 1", help="Prediction: contribution 49, required final 77.5"):
            _load_demo_case_1()
            st.rerun()
    with d2:
        if st.button("Case 2", help="Final score 81, grade B, GPA 3.0"):
            _load_demo_case_2()
            st.rerun()
    ...
    attendance_score = st.slider("Attendance Score", 0, 100, 90, 1,
                                  key="attendance_score",
                                  help="Your attendance score (0–100)")
    st.markdown(f"""<div class='score-pill'>
        <span>📅 Attendance <small style='opacity:.6;'>(20%)</small></span>
        <span class='pill-val'>{attendance_score}</span>
    </div>""", unsafe_allow_html=True)
    ...
    # Mini weight reference
    st.markdown("<div class='sb-label'>⚖️ Weight Reference</div>", unsafe_allow_html=True)
    st.markdown("""
    <div style='font-size:0.76rem; color:#5a4f80; line-height:2; padding:0 2px;'>
        📝 Assignment &nbsp;&nbsp;→ &nbsp;<b style='color:#a78bfa;'>10%</b><br>
        📖 Midterm Exam &nbsp;→ &nbsp;<b style='color:#60a5fa;'>30%</b><br>
        📝 Final Exam &nbsp;&nbsp;&nbsp;→ &nbsp;<b style='color:#f472b6;'>40%</b><br>
        📅 Attendance &nbsp;&nbsp;&nbsp;→ &nbsp;<b style='color:#34d399;'>20%</b>
    </div>
    """, unsafe_allow_html=True)
...
# ──────────────────────────────────────────────────────────────────────────────
assignment_contrib   = assignment_score  * WEIGHTS["Assignments"]
midterm_contrib      = midterm_score     * WEIGHTS["Midterm Exam"]
attendance_contrib   = attendance_score  * WEIGHTS["Attendance"]
current_contribution = assignment_contrib + midterm_contrib + attendance_contrib

if has_final:
    final_contrib   = final_score * WEIGHTS["Final Exam"]
    ...
    final_contrib   = None
    overall_score   = None
    required_final  = (target_score - current_contribution) / WEIGHTS["Final Exam"]
    prediction_mode = True
    if 0 <= required_final <= 100:
        grade, gpa = score_to_grade(target_score)
    elif required_final < 0:
        grade, gpa = score_to_grade(current_contribution + 0 * WEIGHTS["Final Exam"])
    else:
        grade, gpa = "F", 0.0
...
with c1:
    st.metric("📋 Current Contribution", f"{current_contribution:.2f}",
              help="Assignment×10% + Midterm×30% + Attendance×20%")
with c2:
    if has_final:
        st.metric("🏁 Final Score", f"{overall_score:.2f}",
                  help="Assignment×10% + Midterm×30% + Final×40% + Attendance×20%")
    else:
        disp = f"{required_final:.2f}" if required_final is not None else "—"
        st.metric("🎯 Required Final Exam", disp,
...
with left_col:
    st.markdown("<div class='sec-hdr'><span>🏅</span> Grade Result</div>",
                unsafe_allow_html=True)
    score_line = (
        f"Final Score: <b>{overall_score:.2f}</b>" if has_final
        else f"Target Score: <b>{target_score}</b>"
    )
    st.markdown(f"""
    <div class='grade-wrap'>
        <div class='grade-letter grade-{grade}'>{grade}</div>
        <div class='grade-gpa'>GPA &nbsp;<strong>{gpa:.1f}</strong> / 4.0</div>
...
with viz_col:
    ...
    if has_final:
        ...
        labels = ["Assignment (10%)", "Midterm (30%)", "Attendance (20%)", "Final (40%)"]
...
grade_rows = [
    ("90 – 100", "A", "4.0", "Excellent",    "A"),
    ("80 – 89",  "B", "3.0", "Good",         "B"),
    ("70 – 79",  "C", "2.0", "Satisfactory", "C"),
    ("60 – 69",  "D", "1.0", "Passing",      "D"),
    ("Below 60", "F", "0.0", "Failing",      "F"),
]
g1, g2 = st.columns(2, gap="medium")
for i, (rng, ltr, gp, meaning, tag) in enumerate(grade_rows):
    col = g1 if i < 3 else g2
    with col:
        st.markdown(f"""
        <div class='grade-row'>
            <span class='gr-letter grade-{ltr}' style='border-radius:6px;padding:2px 8px;
    ...
    <div class='formula-outer'>
        ...
        <div class='formula-body'>
            <div style='display:flex; flex-wrap:wrap; gap:4px; margin-bottom:14px;'>
                <div class='weight-chip'>📝 Assignment <span class='wc-pct'>10%</span></div>
                <div class='weight-chip'>📖 Midterm <span class='wc-pct'>30%</span></div>
                <div class='weight-chip'>📝 Final Exam <span class='wc-pct'>40%</span></div>
                <div class='weight-chip'>📅 Attendance <span class='wc-pct'>20%</span></div>
            </div>
    ...
    <div class='formula-outer'>
        ...
        <div class='formula-body'>
            <div class='formula-block'>
                ...
                &nbsp;&nbsp;Assignment × 0.10<br>
                &nbsp;+ Midterm × 0.30<br>
                &nbsp;+ Attendance × 0.20<br><br>
                <span class='fb-eq-c'>Required Final Exam =</span><br>
                &nbsp;&nbsp;(Target − Current Contribution) ÷ 0.40
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
